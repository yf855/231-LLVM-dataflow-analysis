...
class DataFlowAnalysis {
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
		...
		for (auto it: InstrToIndex){
			//ignore if index is zero (dummy)
			if(it.second != 0){
				worklist.push_back(it.second);
			}
		}
		//errs()<<"worklist initialization completed \n";

    	// (3) Compute until the work list is empty
    	while (!worklist.empty()) {
    		std::vector<unsigned> incomingEdges;
    		std::vector<unsigned> outgoingEdges;
    		std::vector<Info *> infosOut;
    		
    		unsigned instrIdx = worklist.back();
			//errs()<<"processing instruction index: "<<instrIdx<<"...\n";
    		worklist.pop_back();
    		...
    		flowfunction(instr, incomingEdges, outgoingEdges, infosOut);
			//errs()<<"before instruction index for loop...\n";
    		for (unsigned i = 0; i < infosOut.size(); i++) {
				//errs()<<"processing i: "<<i<<"...\n";
    			Edge outEdge = std::make_pair(instrIdx, outgoingEdges[i]);
    			Info * newInfo = new Info();
    			Info::join(EdgeToInfo[outEdge], infosOut[i], newInfo);
				//ensure termination
				//errs()<<"before if statement...\n";
    			if (!Info::equals(EdgeToInfo[outEdge], newInfo)) {
					//errs()<<"able to evaluate the if statement...\n";
    				EdgeToInfo[outEdge] = newInfo;
    				worklist.push_front(outEdge.second);
    			}
				//errs()<<"moving on to next i...\n";
    		}

			//errs()<<"after instruction index for loop...\n";
    	}
    }
};
